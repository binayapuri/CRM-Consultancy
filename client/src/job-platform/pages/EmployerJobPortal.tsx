...
export default function EmployerJobPortal() {
  ...
  const postJob = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (jobForm.goLiveAt) payload.goLiveAt = new Date(jobForm.goLiveAt).toISOString();
      if (jobForm.listingEndsAt) payload.listingEndsAt = new Date(jobForm.listingEndsAt).toISOString();
  ...
  const saveEmployerEdit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!editingEmployer) return;
    setError('');
    try {
      const res = await authFetch(`/api/jobs/recruiter/employers/${editingEmployer._id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          companyName: editingEmployer.companyName,
          abn: editingEmployer.abn,
          contactName: editingEmployer.contactName,
          contactEmail: editingEmployer.contactEmail,
          contactPhone: editingEmployer.contactPhone,
          website: editingEmployer.website,
          logoUrl: editingEmployer.logoUrl,
          isActive: editingEmployer.isActive !== false,
        }),
      });
      const data = await res.json().catch(() => ({}));
  ...
  const deactivateEmployer = async (id: string) => {
    if (!confirm('Deactivate this company profile? Jobs already posted stay on file; you can no longer select this profile for new posts.')) return;
    try {
      const res = await authFetch(`/api/jobs/recruiter/employers/${id}`, { method: 'DELETE' });
      if (!res.ok) {
  ...
  const addRecruiterEmployer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setEmployerForm({ companyName: '', abn: '', contactName: '', contactEmail: '', contactPhone: '', website: '', logoUrl: '' });
      fetchDashboardData();
    } catch (err: any) {
    ...
    try {
      await authFetch(`/api/jobs/${editingJob._id}`, {
        ...
        body: JSON.stringify({
          ...
          goLiveAt: editingJob.goLiveAt ? new Date(editingJob.goLiveAt).toISOString() : null,
          listingEndsAt: editingJob.listingEndsAt ? new Date(editingJob.listingEndsAt).toISOString() : null,
        }),
      });
      setEditingJob(null);
      fetchDashboardData();
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in-up">
      <div className="mb-8 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-slate-500 mt-1">
            Multi-company profiles (recruiters), brand logos, schedule when listings go live, and manage applicants — similar to a dedicated job board.
          </p>
        </div>
      ...
      {editingEmployer && user?.role === 'RECRUITER' && (
        <form onSubmit={saveEmployerEdit} className="bg-white border border-indigo-200 rounded-2xl p-5 mb-5 grid md:grid-cols-2 gap-3 ring-2 ring-indigo-100">
          <h3 className="md:col-span-2 font-black text-slate-900">Edit company profile</h3>
          <input className="input" value={editingEmployer.companyName} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, companyName: e.target.value }))} required />
          <input className="input" placeholder="ABN" value={editingEmployer.abn || ''} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, abn: e.target.value }))} />
          <input className="input" placeholder="Contact" value={editingEmployer.contactName || ''} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, contactName: e.target.value }))} />
          <input className="input" placeholder="Email" value={editingEmployer.contactEmail || ''} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, contactEmail: e.target.value }))} />
          <input className="input md:col-span-2" placeholder="Phone" value={editingEmployer.contactPhone || ''} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, contactPhone: e.target.value }))} />
          <input className="input md:col-span-2" placeholder="Website" value={editingEmployer.website || ''} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, website: e.target.value }))} />
          <input className="input md:col-span-2" placeholder="Logo URL" value={editingEmployer.logoUrl || ''} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, logoUrl: e.target.value }))} />
          <label className="md:col-span-2 flex items-center gap-2 text-sm font-bold text-slate-700">
            <input type="checkbox" checked={editingEmployer.isActive !== false} onChange={(e) => setEditingEmployer((x: any) => ({ ...x, isActive: e.target.checked }))} /> Active (selectable when posting jobs)
          </label>
          <div className="md:col-span-2 flex gap-2 justify-end">
            <button type="button" onClick={() => setEditingEmployer(null)} className="px-4 py-2 border border-slate-200 rounded-xl font-bold">
              Cancel
            </button>
      ...
      {user?.role === 'RECRUITER' && employers.length > 0 && (
        <div className="mb-6 grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {employers.map((e: any) => (
            <div key={e._id} className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-col gap-3 shadow-sm">
              <div className="flex gap-3 items-start">
                <div className="w-14 h-14 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center shrink-0">
                  {e.logoUrl ? (
                    <img src={e.logoUrl.startsWith('http') ? e.logoUrl : resolveFileUrl(e.logoUrl)} alt="" className="w-full h-full object-contain" />
                  ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500 truncate">{e.contactEmail || e.website || '—'}</p>
                  {e.isActive === false && <span className="text-[10px] font-bold text-rose-600 uppercase">Inactive</span>}
                </div>
              </div>
              <div className="flex gap-2 justify-end pt-1 border-t border-slate-100">
                <button type="button" onClick={() => setEditingEmployer({ ...e })} className="text-xs font-bold text-indigo-600 hover:underline">
                  Edit
                </button>
                <button type="button" onClick={() => deactivateEmployer(e._id)} className="text-xs font-bold text-slate-500 hover:text-rose-600">
                  Deactivate
                </button>
              </div>
            </div>
      ...
      {showEmployerForm && user?.role === 'RECRUITER' && (
        <form onSubmit={addRecruiterEmployer} className="bg-white border border-slate-200 rounded-2xl p-5 mb-5 grid md:grid-cols-2 gap-3">
          ...
          <input className="input md:col-span-2" placeholder="Website" value={employerForm.website} onChange={(e) => setEmployerForm((f) => ({ ...f, website: e.target.value }))} />
          <input className="input md:col-span-2" placeholder="Brand logo URL (https://… or /uploads/…)" value={employerForm.logoUrl} onChange={(e) => setEmployerForm((f) => ({ ...f, logoUrl: e.target.value }))} />
        ...
        <form onSubmit={postJob} className="bg-white border border-slate-200 rounded-2xl p-5 mb-6 grid md:grid-cols-2 gap-3">
          ...
          {user?.role === 'RECRUITER' ? (
            <select className="input" value={jobForm.recruiterEmployerProfileId} onChange={(e) => setJobForm((f) => ({ ...f, recruiterEmployerProfileId: e.target.value }))} required>
              ...
              {employers.filter((e: any) => e.isActive !== false).map((e: any) => <option key={e._id} value={e._id}>{e.companyName}</option>)}
            </select>
          ) : (
          ...
          <div className="md:col-span-2 flex items-center gap-2 text-sm font-black text-slate-800 pt-2">
            <CalendarClock className="w-4 h-4 text-emerald-600" /> Listing schedule
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Go live (optional)</label>
            <input
              type="datetime-local"
              className="input"
              value={jobForm.goLiveAt}
              onChange={(e) => setJobForm((f) => ({ ...f, goLiveAt: e.target.value }))}
            />
            <p className="text-xs text-slate-400 mt-1">Job stays hidden from students until this time.</p>
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 uppercase mb-1">Stop listing (optional)</label>
            <input
              type="datetime-local"
              className="input"
              value={jobForm.listingEndsAt}
              onChange={(e) => setJobForm((f) => ({ ...f, listingEndsAt: e.target.value }))}
            />
          </div>
      ...
      {editingJob && (
        <form onSubmit={saveJobEdit} className="bg-white border border-slate-200 rounded-2xl p-6 mb-6">
          ...
          <div className="grid md:grid-cols-2 gap-3">
            ...
            <div className="md:col-span-2 text-sm font-bold text-slate-800 pt-2">Listing schedule</div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">Go live</label>
              <input
                type="datetime-local"
                className="input"
                value={toLocalInput(editingJob.goLiveAt)}
                onChange={(e) => setEditingJob((j: any) => ({ ...j, goLiveAt: e.target.value ? new Date(e.target.value).toISOString() : null }))}
              />
            </div>
            <div>
              <label className="block text-xs text-slate-500 mb-1">Stop listing</label>
              <input
                type="datetime-local"
                className="input"
                value={toLocalInput(editingJob.listingEndsAt)}
                onChange={(e) => setEditingJob((j: any) => ({ ...j, listingEndsAt: e.target.value ? new Date(e.target.value).toISOString() : null }))}
              />
            </div>
          </div>
