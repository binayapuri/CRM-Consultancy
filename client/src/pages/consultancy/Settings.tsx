              ...
              {(form.emailProfiles || []).map((ep, i) => (
                <div key={ep._id || i} className="p-4 border rounded-lg bg-slate-50 space-y-3">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    ...
                    <div><label className="block text-xs font-medium text-slate-600 mb-1">Password</label><input type="password" value={ep.pass} onChange={e => setForm(f => { const p = [...(f.emailProfiles || [])]; p[i] = { ...p[i], pass: e.target.value }; return { ...f, emailProfiles: p }; })} className="input text-sm" placeholder={ep.pass ? '••••••••' : 'Leave blank to keep'} /><p className="text-xs text-slate-500 mt-1">Gmail: use an App Password (Google Account → Security), not your normal password. Paste the 16 characters with no spaces.</p></div>
