...
export default function LeadForm() {
  ...
  return (
    <div>
      <Link to="/consultancy/leads" className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-900 mb-4">
      ...
      <form onSubmit={handleSubmit} className="card mt-6 max-w-2xl">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium text-slate-700 mb-1">Email</label><input type="email" value={form.profile.email} onChange={e => setForm(f => ({ ...f, profile: { ...f.profile, email: e.target.value } }))} className="input" /></div>
        ...
        <div className="flex gap-4 mt-6">
          ...
          <Link to="/consultancy/leads" className="btn-secondary">Cancel</Link>
        </div>
      </form>
    </div>
  );
