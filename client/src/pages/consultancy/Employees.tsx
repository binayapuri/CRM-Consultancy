...
export default function Employees() {
  ...
  const canAdd = ['SUPER_ADMIN', 'CONSULTANCY_ADMIN'].includes(user?.role || '');
      ...
      {showForm && (
        <div className="card mt-6 max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div><label className="block text-sm font-medium text-slate-700 mb-1">Role</label><select value={form.role} onChange={e => setForm(f => ({ ...f, role: e.target.value }))} className="input"><option value="AGENT">Agent</option><option value="CONSULTANCY_ADMIN">Admin</option></select></div>
          ...
          <tbody>
            {employees.map((emp: any) => (
              <tr key={emp._id} className="border-b border-slate-100 hover:bg-slate-50/50">
                ...
                <td className="px-4 py-3"><span className={`px-2 py-1 rounded text-xs ${emp.role === 'CONSULTANCY_ADMIN' ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'}`}>{emp.role}</span></td>
                ...
                <td className="px-4 py-3 flex gap-2">
                  ...
                  {canDelete && emp._id !== user?.id && <button onClick={() => handleDeactivate(emp._id)} className="p-2 text-red-500 hover:bg-red-50 rounded" title="Deactivate"><UserX className="w-4 h-4" /></button>}
                </td>
              </tr>
            ))}
          </tbody>
