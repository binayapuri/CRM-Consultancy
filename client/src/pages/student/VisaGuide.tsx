...
// ── PR Timeline (labels without emoji; icons rendered in UI) ─────────────────
const PATHWAY_TIMELINE: Record<string, { label: string; timeline: string; key: string }[]> = {
  offshore_study: [
    ...
    { key: 'eoi',      label: 'Lodge EOI in SkillSelect',          timeline: '1 day (wait for invite)' },
    ...
    <div className="w-full space-y-8 animate-fade-in-up">
      ...
      <div>
        <h1 className="text-4xl font-black text-slate-900 tracking-tight flex items-center gap-2">
          <Map className="w-10 h-10 text-indigo-600 shrink-0" aria-hidden /> Australian Visa Guide
        </h1>
      ...
      {activeSection === 'visas' && (
        <div className="space-y-4">
          ...
          {VISAS.map(visa => (
            <div key={visa.code} className="rounded-xl overflow-hidden" style={{ border: expandedVisa === visa.code ? '2px solid #C7D2FE' : '2px solid #E2E8F0', background: 'white', boxShadow: expandedVisa === visa.code ? '0 4px 24px rgba(99,102,241,0.1)' : 'none' }}>
              {/* Card header */}
              <button className="w-full text-left p-6 flex items-center gap-5" onClick={() => toggle(visa.code)}>
                <div className={`w-14 h-14 rounded-lg flex items-center justify-center bg-gradient-to-br ${visa.gradient} shadow-lg shrink-0 text-white`}>
                  {(() => { const Icon = visa.icon; return Icon ? <Icon className="w-7 h-7" aria-hidden /> : null; })()}
                </div>
                    ...
                    <ul className="space-y-1">
                      {visa.conditions.map((c, i) => (
                        <li key={i} className="text-sm text-slate-600 font-medium flex items-center gap-2">
                          {c.startsWith('⚠️') && <AlertTriangle className="w-4 h-4 shrink-0 text-amber-500" aria-hidden />}
                          {c.startsWith('✅') && <CheckCircle2 className="w-4 h-4 shrink-0 text-emerald-500" aria-hidden />}
                          <span>{c.replace(/^⚠️\s*/, '').replace(/^✅\s*/, '')}</span>
                        </li>
                      ))}
                    </ul>
                  </div>

                  <div className="p-5 rounded-lg" style={{ background: '#EEF2FF', border: '1px solid #C7D2FE' }}>
                    <p className="text-sm font-black text-indigo-800 mb-2 uppercase tracking-wider flex items-center gap-2">
                      <Route className="w-4 h-4 shrink-0" aria-hidden /> PR Pathway
                    </p>
                    <p className="text-sm text-indigo-700 font-medium">{visa.prPathway}</p>
                  </div>

                  <div className="p-5 rounded-lg" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0' }}>
                    <p className="text-sm font-black text-slate-700 mb-3 uppercase tracking-wider flex items-center gap-2">
                      <Lightbulb className="w-4 h-4 shrink-0" aria-hidden /> Expert Tips
                    </p>
                    <ul className="space-y-2">
                      {visa.tips.map((t, i) => (
      ...
      {activeSection === 'pathway' && (
        <div className="space-y-6">
          <div className="p-6 rounded-xl" style={{ background: 'linear-gradient(135deg, #0F0E2E, #1a1560)', border: '1px solid rgba(255,255,255,0.1)' }}>
            <h2 className="text-2xl font-black text-white mb-1 flex items-center gap-2">
              <Map className="w-7 h-7 shrink-0 text-white" aria-hidden /> Offshore Student → Australian PR
            </h2>
          ...
          <div className="relative pl-8 border-l-4 border-indigo-100 space-y-0">
            {PATHWAY_TIMELINE.offshore_study.map((step, i) => (
              <div key={step.key} className="relative pb-6">
                ...
                <div className="ml-4 p-4 rounded-lg bg-white" style={{ border: '1px solid #E8EDFB' }}>
      ...
      {activeSection === 'conditions' && (
        <div className="space-y-5">
          {[
            { icon: GraduationCap, title: 'Student Visa 500 — Work Conditions', content: 'You can work up to 48 hours per fortnight during your study session (semester). There is no limit during official university breaks (mid-year, end of year). If you breach this condition, your visa may be cancelled.' },
            { icon: ClipboardList, title: 'Genuine Student (GS) Requirement', content: 'You must demonstrate you are a genuine student. IMMI assesses: your ties to home country, financial capacity, your study background, and whether your study plans make sense. A strong GS statement is essential for offshore students.' },
            { icon: TrendingUp, title: 'Satisfactory Course Progress', content: 'Student visa holders must maintain satisfactory academic progress. If you fail too many units or are excluded, your provider must report you to IMMI, which may trigger a visa cancellation process.' },
            { icon: Briefcase, title: '485 Work Rights — No Limits', content: 'The Graduate Visa (485) grants unlimited work rights. You can work in any job, any number of hours, for any employer. Use this time wisely to build Australian work experience in your nominated occupation.' },
            { icon: Map, title: '491 Regional Obligation', content: 'If you are granted the 491, you must genuinely live and work in a designated regional area. Misrepresenting where you live is fraud and can result in visa cancellation and permanent bar from Australian visas.' },
            { icon: Tag, title: 'Reporting Obligation', content: 'Your visa may have 8107 (work) or 8208 (OSHC) conditions attached. You must not breach these. If your circumstances change (change of address, course, provider), update IMMI via your ImmiAccount.' },
          ].map((item, i) => {
            const Icon = item.icon;
            return (
            <div key={i} className="bg-white p-6 rounded-xl" style={{ border: '1px solid #E8EDFB' }}>
              <h3 className="font-black text-slate-900 text-lg mb-2 flex items-center gap-2">
                <Icon className="w-5 h-5 text-indigo-600 shrink-0" aria-hidden /> {item.title}
              </h3>
