...
export async function renderInvoicePdfBuffer(invoice) {
  return await new Promise((resolve, reject) => {
    try {
      ...
      const docType = String(invoice.documentType || 'INVOICE').toUpperCase();
      const title = docType === 'QUOTE' ? 'QUOTE' : invoice.gstEnabled ? 'TAX INVOICE' : 'INVOICE';
      ...
      doc.font('Helvetica-Bold').fontSize(10).text(`${docType === 'QUOTE' ? 'Quote' : 'Invoice'} No: ${invoice.documentNumber || invoice.invoiceNumber}`, left, 18, { width: contentW, align: 'right' });
      ...
      if (invoice.validUntil) doc.font('Helvetica').fontSize(9).text(`Valid Until: ${fmtDate(invoice.validUntil)}`, left, 48, { width: contentW, align: 'right' });
      doc.restore();
      ...
      if (invoice.title) {
        doc.font('Helvetica-Bold').fontSize(11).fillColor('#0F172A').text(invoice.title, left, doc.y + 8, { width: contentW });
        doc.moveDown(0.9);
      }
      ...
      doc.moveDown(0.8);
      doc.fontSize(8).fillColor('#64748B').text(`Generated by BIGFEW ${docType === 'QUOTE' ? 'Quote' : 'Invoice'} Manager`, { align: 'center' });
      doc.end();
    } catch (e) {
      reject(e);
    }
